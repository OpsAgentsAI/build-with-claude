...
- **If the button dance can't be performed and the device already runs UIFlow2, skip the flash entirely.** When the USB descriptor reads `Cardputer-ADV(UiFlow2)` (visible via `system_profiler SPUSBDataType | grep Cardputer` on macOS), UIFlow is already installed — re-flashing isn't required to pick up new apps. Run `python3 scripts/install_apps.py --port <PORT> --src buddy` directly: it pushes apps over the existing REPL, sets `boot_option=2`, and reboots via `repl_reset()` — no BtnG0+BtnRST required. The right escape hatch when (a) the back-of-device buttons are physically inaccessible on some case revisions, (b) the user has exhausted the 10-attempt retry loop, or (c) they only need to refresh apps and don't want the ~280 s re-flash. `onboard.py` currently always routes through FLASH first; an enhancement could detect `Cardputer-ADV(UiFlow2)` in the USB descriptor and offer "skip flash?" before requiring the button dance.
